...
func (c Credentials) SignUrl(request *http.Request) string {
	...
	sdt := ldt[:8]
	meta := new(metadata)
	meta.date, meta.service, meta.region, meta.algorithm = sdt, c.Service, c.Region, "HMAC-SHA256"
	meta.credentialScope = concat("/", meta.date, meta.region, meta.service, "request")

	// Determine host for signing
	host := request.Host
	if host == "" {
	...
	// Task 1
	hashedCanonReq := hashedSimpleCanonicalRequestV4(request, query, meta, host)
	...
	// Add X-Signature and X-Security-Token AFTER signature computation
...
func hashedSimpleCanonicalRequestV4(request *http.Request, query url.Values, meta *metadata, host string) string {
	...
	// Build canonical headers section
	var canonicalHeaders string
	if meta.signedHeaders != "" && host != "" {
		h := host
	...
	canonicalRequest := concat("\n", request.Method, normuri(request.URL.Path), normquery(query), canonicalHeaders, meta.signedHeaders, payloadHash)
