...
ou can use credentials from a `credential_process` in a variety of ways.

One way is to setup your shared config file, located in the default
location, with the `credential_process` key and the command you want to be
called. You also need to set the VOLCSTACK_SDK_LOAD_CONFIG environment variable
(e.g., `export VOLCSTACK_SDK_LOAD_CONFIG=1`) to use the shared config file.
...
process will not be used.
    ...
    sess, _ := session.NewSession(&volcstack.Config{
        Region: volcstack.String("us-east-1")},
    )
...
retrieve credentials:
    ...
    svc := s3.New(sess, &volcstack.Config{Credentials: creds})
...
func (p *ProcessProvider) Retrieve() (credentials.Value, error) {
	...
	if err = json.Unmarshal(out, resp); err != nil {
		return credentials.Value{ProviderName: ProviderName}, volcstackerr.New(
...
func (p *ProcessProvider) executeCredentialProcess() ([]byte, error) {
	...
	for !finished {
		...
		case execError := <-execCh:
			...
			if errors != nil {
				return output.Bytes(), volcstackerr.NewBatchError(
					ErrCodeProcessProviderExecution,
