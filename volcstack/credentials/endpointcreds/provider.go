...
import (
	...
	"github.com/volcengine/volcstack-go-sdk/private/protocol/json/jsonutil"
...
type Provider struct {
	...
	// Requires a Volcstack Client to make HTTP requests to the endpoint with.
	// the Endpoint the request will be made to is provided by the volcstack.Config's
	// Endpoint value.
...
// NewProviderClient returns a credentials Provider for retrieving Volcstack credentials
...
func (p *Provider) Retrieve() (credentials.Value, error) {
	...
	if err != nil {
		return credentials.Value{ProviderName: ProviderName},
			volcstackerr.New("CredentialsEndpointError", "failed to load credentials", err)
	}
...
func unmarshalError(r *request.Request) {
	...
	// Response volcstackbody format is not consistent between metadata endpoints.
	// Grab the error message as a string and include that as the source error
	r.Error = volcstackerr.New(errOut.Code, errOut.Message, nil)
}
