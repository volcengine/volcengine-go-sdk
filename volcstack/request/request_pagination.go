...
func (p *Pagination) HasNextPage() bool {
	...
	if p.EndPageOnSameToken {
		return hasNextPage && !volcstackutil.DeepEqual(p.nextTokens, p.prevTokens)
	}
	return hasNextPage
}
...
func (p *Pagination) Next() bool {
	...
	if p.started {
		for i, intok := range req.Operation.InputTokens {
			volcstackutil.SetValueAtPath(req.Params, intok, p.nextTokens[i])
		}
	}
	p.started = true
...
func (r *Request) nextPageTokens() []interface{} {
	...
	if r.Operation.TruncationToken != "" {
		tr, _ := volcstackutil.ValuesAtPath(r.Data, r.Operation.TruncationToken)
		if len(tr) == 0 {
			return nil
		}
	...
	for _, outToken := range r.Operation.OutputTokens {
		vs, _ := volcstackutil.ValuesAtPath(r.Data, outToken)
		if len(vs) == 0 {
		...
		switch tv := v.(type) {
		case *string:
			if len(volcstackutil.StringValue(tv)) == 0 {
				tokens = append(tokens, nil)
				continue
			}
		case string:
...
func logDeprecatedf(logger volcstack.Logger, flag *int32, msg string) {
	if logger == nil {
		return
	}
...
func (r *Request) NextPage() *Request {
	...
	nr := New(r.Config, r.ClientInfo, r.Handlers, r.Retryer, r.Operation, volcstackutil.CopyOf(r.Params), data)
	for i, intok := range nr.Operation.InputTokens {
		volcstackutil.SetValueAtPath(nr.Params, intok, tokens[i])
	}
	return nr
}
