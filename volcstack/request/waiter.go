...
func WithWaiterLogger(logger volcstack.Logger) WaiterOption {
	return func(w *Waiter) {
		w.Logger = logger
	}
}
...
type Waiter struct {
	...
	SleepWithContext func(volcstack.Context, time.Duration) error
}
...
// Use volcstack.BackgroundContext if no context is available.
//
...
func (w Waiter) WaitWithContext(ctx volcstack.Context) error {
	...
	for attempt := 1; ; attempt++ {
		...
		if sleepFn := req.Config.SleepDelay; sleepFn != nil {
			...
			if sleepCtxFn == nil {
				sleepCtxFn = volcstack.SleepWithContext
			}

			if err := sleepCtxFn(ctx, delay); err != nil {
				return volcstackerr.New(CanceledErrorCode, "waiter context canceled", err)
			}
		}
	}

	return volcstackerr.New(WaiterResourceNotReadyErrorCode, "exceeded wait attempts", nil)
}
...
func (a *WaiterAcceptor) match(name string, l volcstack.Logger, req *Request, err error) (bool, error) {
	result := false
	var vals []interface{}
	...
	case PathAllWaiterMatch, PathWaiterMatch:
		...
		vals, _ = volcstackutil.ValuesAtPath(req.Data, a.Argument)
		if len(vals) == 0 {
			break
		}
		result = true
		for _, val := range vals {
			if !volcstackutil.DeepEqual(val, a.Expected) {
				result = false
				break
			}
		}
	...
	case ErrorWaiterMatch:
		if aerr, ok := err.(volcstackerr.Error); ok {
	...
	case FailureWaiterState:
		...
		return true, volcstackerr.New(WaiterResourceNotReadyErrorCode,
...
func waiterLogf(logger volcstack.Logger, msg string, args ...interface{}) {
	if logger != nil {
