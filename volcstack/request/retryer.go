...
// value for chaining.
func WithRetryer(cfg *volcstack.Config, retryer Retryer) *volcstack.Config {
	cfg.Retryer = retryer
	return cfg
}
...
func isNestedErrorRetryable(parentErr volcstackerr.Error) bool {
	if parentErr == nil {
		return false
	}
...
func shouldRetryError(origErr error) bool {
	...
	case nil:
		// `volcstackerr.Error.OrigErr()` can be nil, meaning there was an error but
...
func IsErrorThrottle(err error) bool {
	if aerr, ok := err.(volcstackerr.Error); ok && aerr != nil {
