...
var timeoutErr = volcstackerr.New(
	...
	"read on volcstackbody has reached the timeout limit",
	nil,
)
...
// timeoutReadCloser will handle volcstackbody reads that take too long.
...
func adaptToResponseTimeoutError(req *Request) {
	if err, ok := req.Error.(volcstackerr.Error); ok {
		aerr, ok := err.OrigErr().(volcstackerr.Error)
...
// WithResponseReadTimeout is a request option that will wrap the volcstackbody in a timeout read closer.
