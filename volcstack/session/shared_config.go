...
const (
	...
	accessKeyIDKey  = `volcstack_access_key_id`     // group required
	secretAccessKey = `volcstack_secret_access_key` // group required
	sessionTokenKey = `volcstack_session_token`     // optional
...
type sharedConfig struct {
	// Credentials values from the config file. Both volcstack_access_key_id and
	// volcstack_secret_access_key must be provided together in the same file to be
	// considered valid. The values will be ignored if not a complete group.
	// volcstack_session_token is an optional field that can be provided if both of
...
func loadSharedConfigIniFiles(filenames []string) ([]sharedConfigFile, error) {
	...
	for _, filename := range filenames {
		...
		if aerr, ok := err.(volcstackerr.Error); ok && aerr.Code() == ini.ErrCodeUnableToReadFile {
...
// example if a config file only includes volcstack_access_key_id but no
// volcstack_secret_access_key the volcstack_access_key_id will be ignored.
...
func (e SharedConfigLoadError) Error() string {
	return volcstackerr.SprintError(e.Code(), e.Message(), "", e.Err)
}
...
func (e SharedConfigAssumeRoleError) Error() string {
	return volcstackerr.SprintError(e.Code(), e.Message(), "", nil)
}
