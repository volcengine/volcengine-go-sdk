...
type envConfig struct {
	...
	// $HOME/.volcstack/credentials on Linux/Unix based systems, and
	// %USERPROFILE%\.volcstack\credentials on Windows.
	//
	...
	// $HOME/.volcstack/config on Linux/Unix based systems, and
	// %USERPROFILE%\.volcstack\config on Windows.
	//
	...
	//
	// Setting a custom HTTPClient in the volcstack.Config options will override this setting.
...
func envConfigLoad(enableSharedConfig bool) envConfig {
	...
	if len(cfg.enableEndpointDiscovery) > 0 {
		cfg.EnableEndpointDiscovery = volcstack.Bool(cfg.enableEndpointDiscovery != "false")
	}
