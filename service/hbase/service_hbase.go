...
// HBASE provides the API operation methods for making requests to
// HBASE. See this package's package overview docs
// for details on the service.
//
// HBASE methods are safe to use concurrently. It is not safe to
...
const (
	ServiceName = "hbase"     // Name of service.
	EndpointsID = ServiceName // ID to lookup a service endpoint with.
	ServiceID   = "hbase"     // ServiceID is a unique identifer of a specific service.
)
...
func newClient(cfg volcengine.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *HBASE {
	svc := &HBASE{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a HBASE operation and runs any
// custom request initialization.
func (c *HBASE) newRequest(op *request.Operation, params, data interface{}) *request.Request {
